...
const inlineAnkiWorkerPlugin = {
  name: 'inline-anki-worker-plugin',
  setup(build) {
    build.onResolve({ filter: /^virtual:anki-worker-script$/ }, () => ({
      path: 'virtual:anki-worker-script',
      namespace: 'inline-anki-worker',
    }))
    build.onLoad(
      { filter: /^virtual:anki-worker-script$/, namespace: 'inline-anki-worker' },
      async () => {
        const result = await esbuild.build({
          entryPoints: ['src/core/learning/anki/worker.ts'],
          bundle: true,
          write: false,
          format: 'iife',
          platform: 'browser',
