...
export type QwenOAuthCredential = {
  accessToken: string
  refreshToken: string
  expiresAt: number
  resourceUrl: string
...
const CREDENTIAL_DIR_NAME = 'qwen-oauth'
const DEFAULT_PROVIDER_ID = 'qwen-oauth'
const EXPIRY_BUFFER_MS = 30_000

const encodeProviderId = (providerId: string): string =>
  encodeURIComponent(providerId)

export class QwenOAuthStore {
  ...
  async get(): Promise<QwenOAuthCredential | null> {
    ...
    try {
      ...
      const parsed = JSON.parse(raw) as Partial<QwenOAuthCredential>
      if (
  ...
  async set(credential: QwenOAuthCredential): Promise<void> {
  ...
  isExpired(credential: Pick<QwenOAuthCredential, 'expiresAt'>): boolean {
