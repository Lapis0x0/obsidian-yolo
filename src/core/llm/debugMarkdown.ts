...
const EMBEDDING_INPUT_EDGE_CHARS = 12
...
function abbreviateEmbeddingText(text: string): string {
  if (text.length <= EMBEDDING_INPUT_EDGE_CHARS * 2) {
    return text
  }

  return `[OMITTED ${text.slice(0, EMBEDDING_INPUT_EDGE_CHARS)}...${text.slice(
    -EMBEDDING_INPUT_EDGE_CHARS,
  )}]`
}
...
function abbreviateEmbeddingArray(values: unknown[]): unknown {
  if (values.length <= EMBEDDING_INPUT_EDGE_CHARS * 2) {
    return values.map((item) => abbreviateEmbeddingInputValue(item))
  }

  const prefix = values
    .slice(0, EMBEDDING_INPUT_EDGE_CHARS)
    .map(formatEmbeddingArrayEdge)
    .join(', ')
  const suffix = values
    .slice(-EMBEDDING_INPUT_EDGE_CHARS)
    .map(formatEmbeddingArrayEdge)
    .join(', ')
  return `[OMITTED ${prefix}...${suffix}]`
}
...
function formatBody({
  ...
  if (embeddingRequest) {
    ...
    if (abbreviated) {
      return {
        ...
        formatted: true,
        formatNote:
          '(pretty-printed; embedding input text and embedding vectors are omitted for display after the first and last 12 items)',
      }
    }
  }
  ...
  return {
    content:
      typeof bodyWithBase64Omitted === 'string'
        ? bodyWithBase64Omitted
        : body,
    language: 'text',
    formatted: false,
  }
  ...
  const unrelatedSections =
    otherRequestEntries.length > 0
      ? [
          ...
          '## Other Requests',
          '',
          'These requests were captured during this debug window, but they are not part of the main chat response itself. Title generation requests are listed here instead of as separate Subrequests. Embedding requests are also listed here because they were not initiated by the chat response itself.',
          '',
