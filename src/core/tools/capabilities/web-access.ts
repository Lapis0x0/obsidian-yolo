...
// `id: 'web_access'` is a new capability id, independent of the old
// `web_ops` group-name string.
//
...
//
// hasSettings: true — ⚠️ easy to get wrong: `AgentToolsModal.tsx:117`'s `hasSettings` ternary only
...
// match (the exact bug this whole registry exists to make impossible).
// `CAPABILITY_SETTINGS_LAUNCHERS.web_access` wires this explicitly instead — see `capabilitySettingsLaunchers.ts`.
