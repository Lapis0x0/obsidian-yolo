import { createBrowserAnkiWorkerHost } from './browserAnkiWorkerHost'

describe('createBrowserAnkiWorkerHost', () => {
  ...
  it('bridges events and revokes the URL exactly once on termination', () => {
    const worker = createBrowserAnkiWorkerHost().spawn<string, string>('source')
  ...
  it('revokes the URL if Worker construction throws', () => {
    ...
    expect(() => createBrowserAnkiWorkerHost().spawn('source')).toThrow(
      'construction failed',
    )
    expect(revokeObjectURL).toHaveBeenCalledWith('blob:anki-worker')
  })
