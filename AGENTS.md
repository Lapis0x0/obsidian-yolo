...
- `src/core/agent/` owns the shared native agent runtime, tool gateway, conversation service, subagents, and background tasks. `NativeAgentRuntime` is the one orchestration; its consumers are `AgentService` (chat surfaces), `YoloAgentApiService` in `agent-api.ts` (programmatic callers, modules included), and `subagent/runner.ts`. A caller without a chat view goes through `agent-api.ts` and states a trust tier as `capability` (`none` / `vault-read` / `vault-write`, in `capability-profile.ts`) instead of hand-assembling tools or an `AgentService.run` input; chat modes resolve through `resolveChatModeRuntime` (`chat-runtime-profiles.ts`).
