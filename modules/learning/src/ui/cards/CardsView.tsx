...
import {
  ...
  type CardFileError,
  CardFileFormatError,
  type LearningCardFileStore,
  type ProjectCardScanResult,
  parseCardFile,
} from '../../domain/cardFile'
import type { LearningVaultReadApi } from '../../domain/learningVaultReadApi'
import { fsrsStateToMastery, isDue } from '../../domain/srs/masteryMapping'
import type { LearningSrsStore } from '../../domain/srs/srsStore'
import type {
  CardScheduling,
  ReviewRating,
  SrsCardState,
...
import { YoloPopoverContent } from './OwnerDocumentPopover'
...
export type LearningText = (keyPath: string, fallback?: string) => string

export type CardsViewServices = {
  ...
  cardFiles: Pick<
    ...
    | 'updateCard'
  >
  markdown: CardMarkdownService
  showNotice(message: string): void
}

type CardsUiContextValue = {
  services: CardsViewServices
  t: LearningText
  ownerDocument: Document
}
const CardsUiContext = createContext<CardsUiContextValue | null>(null)

function useCardsUi(): CardsUiContextValue {
  const value = useContext(CardsUiContext)
  if (!value) throw new Error('CardsView requires services and t')
  return value
}

function getOwnerWindow(ownerDocument: Document): Window {
  const ownerWindow = ownerDocument.defaultView
  if (!ownerWindow) throw new Error('CardsView ownerDocument has no window')
  return ownerWindow
}

function createOwnerDomRect(
  ownerDocument: Document,
  init: Partial<DOMRectInit> = {},
): DOMRect {
  const DomRect = (
    getOwnerWindow(ownerDocument) as Window & { DOMRect: typeof DOMRect }
  ).DOMRect
...
export function CardsView({
  ...
  return (
    <CardsUiContext.Provider value={{ services, t, ownerDocument }}>
      <CardsViewContent
        project={project}
        generation={generation}
        mode={mode}
        onModeChange={onModeChange}
        onStudyCountChange={onStudyCountChange}
...
function useProjectCards(
  ...
  const { ownerDocument, services, t } = useCardsUi()
  ...
  const { now, refreshNow } = useReviewClock(
    cards,
    projectPausedAt,
    ownerDocument,
  )
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const projectScan = await services.scanProjectCards(
        project.folderPath,
        project.chapters.map((chapter) =>
          project.kind === 'cards'
            ? (chapter as VaultCardChapter).cardsFilePath
  ...
  }, [generation, project, refreshToken, services, t, vault, writing])
...
function useReviewClock(
  ...
  useEffect(() => {
    ...
    const timer = ownerWindow.setTimeout(refreshNow, delay)
    const onVisibilityChange = () => {
      if (ownerDocument.visibilityState === 'visible') refreshNow()
    }
    ownerDocument.addEventListener('visibilitychange', onVisibilityChange)
    ownerWindow.addEventListener('focus', refreshNow)
    return () => {
      ownerWindow.clearTimeout(timer)
      ownerDocument.removeEventListener('visibilitychange', onVisibilityChange)
      ownerWindow.removeEventListener('focus', refreshNow)
    }
  }, [cards, now, ownerDocument, pausedAt, refreshNow])
...
function BrowseMode({
  ...
  const fileStore = services.cardFiles
  ...
  useEffect(
    () => () => {
      if (dragMoveFrameRef.current !== null) {
        ownerWindow.cancelAnimationFrame(dragMoveFrameRef.current)
      }
      if (dropSettleTimerRef.current !== null) {
        ownerWindow.clearTimeout(dropSettleTimerRef.current)
      }
      if (marqueeScrollFrameRef.current !== null) {
        ownerWindow.cancelAnimationFrame(marqueeScrollFrameRef.current)
      }
    },
    [],
  )
  ...
  const startMarqueeAutoScroll = useCallback(() => {
    ...
    const tick = () => {
      ...
      marqueeScrollFrameRef.current = ownerWindow.requestAnimationFrame(tick)
    }
    marqueeScrollFrameRef.current = ownerWindow.requestAnimationFrame(tick)
  ...
  const handleMarqueePointerDown = (event: PointerEvent<HTMLDivElement>) => {
    ...
    if (
      ...
      !ownerWindow.matchMedia('(hover: hover) and (pointer: fine)').matches ||
      activeCardId ||
  ...
  useEffect(() => {
    const eventDocument = dragAreaRef.current?.ownerDocument ?? ownerDocument
  ...
  }, [batchSelectedCardIds.size, ownerDocument, stopMarquee])
  ...
  const handleDeleteCards = async (targetCards: Card[]) => {
    ...
    const deleted = await withWrite(async () => {
      ...
      try {
        await services.srs.removeCards(
          project.slug,
  ...
  const handleQuickReviewCards = async (
    ...
    setWriting(true)
    try {
      await services.srs.reviewCards(project.slug, cardIds, rating, new Date())
      refresh()
    } catch (reviewError) {
  ...
  const handleDragEnd = ({ active, over }: DragEndEvent) => {
    ...
    if (ownerWindow.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      ...
      dropSettleTimerRef.current = ownerWindow.setTimeout(() => {
        dropSettleTimerRef.current = null
        setActiveCardId(null)
        setActiveCardHeight(null)
        setActiveDragCardIds([])
  ...
  const handleDragMove = ({ active, collisions, over }: DragMoveEvent) => {
    ...
    dragMoveFrameRef.current = ownerWindow.requestAnimationFrame(() => {
      dragMoveFrameRef.current = null
...
function BrowseCard({
  ...
  const menuAnchorRef = useRef({
    getBoundingClientRect: () => createOwnerDomRect(ownerDocument),
  })
  ...
  const openMenuAt = (x: number, y: number) => {
    menuAnchorRef.current = {
      getBoundingClientRect: () => createOwnerDomRect(ownerDocument, { x, y }),
    }
...
>(function CardInspector(
  ...
  const { ownerDocument, t } = useCardsUi()
  const ownerWindow = getOwnerWindow(ownerDocument)
  ...
  const persistLatest = useCallback(async (): Promise<boolean> => {
    if (saveTimerRef.current !== null) {
      ownerWindow.clearTimeout(saveTimerRef.current)
  ...
  const queueSave = (content: { front: string; back: string }) => {
    ...
    saveTimerRef.current = ownerWindow.setTimeout(() => {
      saveTimerRef.current = null
      void persistLatest()
    }, 600)
  }
...
function ReviewMode({
  ...
  useEffect(() => {
    ...
    const loadScheduling = async () => {
      try {
        const scheduling = await services.srs.getCardScheduling(
          projectSlug,
          card.id,
          new Date(),
        )
  ...
  const commitGrade = useCallback(
    async (grade: ReviewGrade) => {
      ...
      let result
      try {
        result = await services.srs.reviewCard(
          projectSlug,
          card.id,
          grade,
          new Date(),
      ...
      const schedule = (fn: () => void, ms: number) => {
        timersRef.current.push(ownerWindow.setTimeout(fn, ms))
      }
      ...
      schedule(() => {
        ...
        ownerWindow.requestAnimationFrame(() => {
          ownerWindow.requestAnimationFrame(() => setPeeksSettling(false))
        })
      }, EXIT_MS)
